...
use crate::storage::execution_binding::prepare_execution_binding_with_path;
...
fn breakage_contract_task_without_profile_id_uses_first_profile() {
    ...
    crate::task::state::update_task_runtime_snapshot(&db_path, &task_id, None).expect("clear");
...
fn breakage_contract_binding_tamper_invalidates_snapshot() {
    ...
    // Update binding, simulating crate::task::runtime_service::update_task_runtime_context
    crate::task::repository::update_task_engine(&db_path, &task_id, "eng1", Some("custom")).unwrap();
    crate::task::state::update_task_runtime_snapshot(&db_path, &task_id, None).unwrap();
